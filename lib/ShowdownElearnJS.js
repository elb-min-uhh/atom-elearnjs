    ...
    filter: (text, converter) => {
        ...
        if(match && match.length) {
            ...
            text = text.replace(/(\|{3,5})(?!\|)([^\/]*)(\/{3,5})/g, (wholeMatch, before, heading, after) => {
                ...
                return `</section>${replacement}<section markdown="1" name="${heading}" ${clazz}>`
            });
            text += "</section>";
        }
...
var elearnPdfBody = () => {
    return [addSectionOnHeading, cleanNoSectionComment, pdfSectionSyntax, removeMetaBlock, removeImprintBlock, cleanEmptyParagraphs];
}
