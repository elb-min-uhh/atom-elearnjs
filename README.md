...
If you want to use `|` or `\` as a character wihtin the sections name, you have to
escape it with an `\`. So `|` becomes `\|` and `\` becomes `\\` like this
`||| Example \| Section 1|||`. Using normal markdown headings with autogenerated
sections like `## Example | Section 1` is still possible without any escaping.
