...
const addSectionOnHeading = {
    ...
    filter: (text, converter) => {
        const headingDepth = atom.config.get('atom-elearnjs.newSectionOnHeadingDepth');

        // parse headings
        if(atom.config.get('atom-elearnjs.newSectionOnHeading')) {
...
var elearnPdfBody = () => {
    return [removeSectionSyntax, removeMetaBlock, removeImprintBlock, cleanEmptyParagraphs];
}
