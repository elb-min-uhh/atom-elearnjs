...
class FileWriter {
    ...
    */
    saveHTML(filePath, callback) {
        ...
        // TODO if Extension Option is set to "Automatic Detection" scan for them
        console.log(
        ...
        self.openHTMLExportOptions(atom.config.get('atom-elearnjs.generalConfig.displayExportOptions'), (val, opts) => {
            if(!val) {
                if(callback) callback();
                return;
            }
            ...
            FileWriter.readFile(path.resolve(__dirname + '/../assets/elearnjs/template.html'), (data) => {
                ...
                var fileContent = data.replace(/\$\$meta\$\$/, () => {return meta})
                    .replace(/\$\$extensions\$\$/, () => {return ExtensionManager.getHTMLAssetStrings(true, true, true, true);})
                ...
                self.saveToFilePath(fileContent, filePath, extractFiles, {
                    // TODO make export option
                    includeQuiz: true,
    ...
    */
    savePDF(filePath, callback) {
        ...
        FileWriter.readFile(path.resolve(__dirname + '/../assets/elearnjs/template_pdf.html'), (data) => {
            // data => template.html
            var fileContent = data.replace(/\$\$meta\$\$/, () => {return meta})
                .replace(/\$\$extensions\$\$/, () => {return ExtensionManager.getPDFAssetStrings(true, true, true, true);})
    ...
    */
    getPdfOutputOptions(renderDelay) {
        ...
        return {
            // USE OPTIONS HERE
            "format": "A4",
            "base": "file:///" + self.getFileDir().replace(/\\/g, "/") + "/",
            "border": {
    ...
    */
    openHTMLExportOptions(forcePrompt, callback) {
        const self = this;
        ...
        var displayExportOptions = OptionMenuManager.getCheckBoxLabel("Always display export options",
        ...
        self.optionMenuManager.open({
            ...
            if(val) {
                ...
                if(displayExportOptions.checkbox.checked
                        !== atom.config.get('atom-elearnjs.generalConfig.displayExportOptions')) {
                    atom.config.set('atom-elearnjs.generalConfig.displayExportOptions',
                        displayExportOptions.checkbox.checked);
                }
            }
    ...
    */
    deserialize(state) {
        ...
        if(atom.config.get('atom-elearnjs.generalConfig.keepExportSettings')
            && state.lastHTMLExportOptions) this.lastHTMLExportOptions = state.lastHTMLExportOptions;
    }
