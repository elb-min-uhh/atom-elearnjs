            ...
            if(match && match.length) {
                text = text.replace(/^(#{1,6})(?!#)(?!<!--no-section-->)[ \t]*(.+?)[ \t]*#*$/gm, (wholeMatch, type, content) => {
                    ...
                    if(type.length <= headingDepth
                        ...
                        content = content.replace(/<!--hide-in-overview-->/g, "");

                        // default section
                        if(!useSubSections
                                || type.length < subSection) {
                            ret = `|||${content}///${hideInOverview}\n` + ret;
                        }
...
const replaceSectionSyntax = {
    type: 'lang',
    filter: (text, converter) => {
        var parseSection = (wholeMatch, before, heading, after, addition) => {
            ...
            var clazz = sub || hide ? `class="${sub}${sub ? ' ' : ''}${hide}"` : '';

            return `<section markdown="1" name="${heading}" ${clazz}>\n`;
        };

        var match = text.match(/(\|{3,5})(?!\|)([^\/]*)(\/{3,5})(.*)\n/g);
        if(match && match.length) {
            // replace only first
            text = text.replace(/(\|{3,5})(?!\|)([^\/]*)(\/{3,5})(.*)\n/, parseSection);
            // replace all following
            text = text.replace(/(\|{3,5})(?!\|)([^\/]*)(\/{3,5})(.*)\n/g, (wholeMatch, before, heading, after, addition) => {
                return `</section>${parseSection(wholeMatch, before, heading, after, addition)}`;
            });
            text += "</section>";
        }
...
const pdfSectionSyntax = {
    ...
    filter: (text, converter) => {
        ...
        if(match && match.length) {
            ...
            // replace all following
            text = text.replace(/(\|{3,5})(?!\|)([^\/]*)(\/{3,5})(.*)\n/g, (wholeMatch, before, heading, after, addition) => {
                return `</section>${replacement}${parseSection(wholeMatch, before, heading, after, addition)}`;
            });
            text += "</section>";
        }
...
const cleanNoSectionComment = {
    ...
    listeners: {
        'headers.before': (event, text, options, globals) => {
            text = text.replace(/\s*<!--no-section-->/g, "");
            return text;
        }
    }
};
...
const cleanHideInOverviewComment = {
    ...
    listeners: {
        'headers.before': (event, text, options, globals) => {
            text = text.replace(/\s*<!--hide-in-overview-->/g, "");
            return text;
        }
    }
};
